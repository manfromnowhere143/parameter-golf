...
class HybridGDN(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor,
                ...
                emt_alpha: float = 0.0,
                emt_temperature: float = 1.0) -> Tensor:
        """Forward pass returning cross-entropy loss.

        Aweb EMT extension: when `teacher_logits` is provided AND `emt_alpha > 0`,
        loss = (1-α)·CE(target) + α·KL(student || teacher.detach()).
        Teacher logits must be pre-computed by the caller (typically from a
        separate EMA-weighted teacher model under torch.no_grad()).
        Temperature scales both student and teacher logits before softmax (T=1 default).
        """
        ...
        if teacher_logits is None or emt_alpha <= 0.0:
            return ce_loss
        # EMT: KL(student || teacher) with temperature scaling.
        # Student log-probs computed with grad; teacher probs detached.
        T = emt_temperature
        t_logits = teacher_logits.reshape(-1, teacher_logits.size(-1)).float()
        log_p_student = F.log_softmax(logits.float() / T, dim=-1)
        with torch.no_grad():
            p_teacher = F.softmax(t_logits / T, dim=-1)
        # batchmean divides by batch size (B*Tlen here since we flattened);
        # multiply by T*T to recover Hinton-style soft-target gradient magnitude.
        kl_loss = F.kl_div(log_p_student, p_teacher, reduction="batchmean") * (T * T)
        return (1.0 - emt_alpha) * ce_loss + emt_alpha * kl_loss
