...
class Hyperparameters:
    ...
    # EMT (EMA-Teacher Distillation) — Aweb signature contribution.
...
def main():
    ...
    # ─── EMT (EMA-Teacher Distillation) state ────────────────────────────
    ...
    def emt_alpha_for(step: int) -> float:
        ...
        if not args.emt_enabled:
            return 0.0
        if step < emt_warmup_start_step:
            return 0.0
        if step >= emt_warmup_end_step:
            return args.emt_alpha_max
        frac = (step - emt_warmup_start_step) / max(1, emt_warmup_end_step - emt_warmup_start_step)
        return frac * args.emt_alpha_max
    ...
    for step in range(start_step + 1, args.iterations + 1):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                loss = model(x_micro, y_micro,
                             ...
                             emt_alpha=cur_alpha,
                             emt_temperature=args.emt_temperature)
        ...
        if step % args.train_log_every == 0 or step <= 10:
            ...
            emt_str = f" | emt_α {cur_alpha:.3f}" if teacher_model is not None else ""
            log0(f"step {step:5d}/{args.iterations} | loss {avg_loss:.4f} | lr_mul {lr_mul:.4f} | "
                 f"mom {current_muon_momentum:.3f} | {steps_per_sec:.2f} steps/s | {elapsed:.0f}s{emt_str}")
            running_loss = 0.0
            loss_count = 0
