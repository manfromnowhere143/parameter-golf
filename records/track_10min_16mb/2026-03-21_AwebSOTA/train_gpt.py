...
FP16_KEEP_NAME_PATTERNS = ("tok_emb",)  # Keep embeddings in FP16 — critical for quality

def quantize_float_tensor(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Int6 per-row quantization: [-32, 31] range by default (6-bit)."""
    t32 = t.float()
    if t32.ndim == 2:
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # Keep embeddings in FP16 — disproportionately damaged by quantization
...
class SmearGate(nn.Module):
    """Blend each token embedding with previous token via learned sigmoid gate."""
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bigram_hash = BigramHashEmbedding(bigram_hash_buckets, bigram_hash_dim, model_dim) if bigram_hash_buckets > 0 else None
...
def main() -> None:
    ...
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        ...
        avg_sd = {n: (t / swa_count).to(dtype=current_sd[n].dtype) for n, t in swa_state.items()}
